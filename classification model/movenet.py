...
def extract_keypoints_from_frame(frame):
    """
    Takes an OpenCV BGR frame and returns flattened keypoints
    shape: (1, 34)
    """
    ...
    # Initialize movenet model ONCE
    global _movenet_model
    if '_movenet_model' not in globals():
        _movenet_model = Movenet("movenet_thunder.tflite")

    person = _movenet_model.detect(image_rgb)

    keypoints = []
    for kp in person.keypoints:
        keypoints.append(kp.coordinate.x)
        keypoints.append(kp.coordinate.y)

    # Convert to numpy shape (1, N)
